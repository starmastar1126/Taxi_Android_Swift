...
-(id)initWithTextField:(UITextField *)tf withData:(NSArray*) data
{
    ...
    if (self) {
        ...
        UIImage* img = [UIImage imageNamed:@""];   // non-CocoaPods--ic_arrow_down
        if (img == nil) img = [UIImage imageNamed:@""]; // CocoaPods--DownPicker.bundle/downArrow.png
...
//        self->textField.placeholder = self->placeholder;
        if(self->placeholder.length == 0){
            self->placeholder = self->textField.text;
            _previousSelectedString = self->placeholder;
        }else{
            self->textField.placeholder = self->placeholder;
        }
    }
...
- (void)showPickerView:(UITextField *) aTextField
{
    [self showPicker:aTextField];
}
