...
@objc @IBDesignable open class CircleProgressView: UIView {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if roundedCap {
            ...
            progressPath.addArc(withCenter: capCenter, radius: trackWidth * 0.5, startAngle: endAngle, endAngle: endAngle + CGFloat(CGFloat.pi), clockwise: !clockwise)
        }
        ...
        let capCenter = CGPoint(x: center.x + r * cos(startAngle), y: center.y + r * sin(startAngle))
            ...
            progressPath.addArc(withCenter: capCenter, radius: trackWidth * 0.5, startAngle: startAngle, endAngle: startAngle + CGFloat(CGFloat.pi), clockwise: clockwise)
        }
        progressPath.close()
