...
-(NSString*)getGoogleMapLngCode
{
    NSString* gMapLngCode = [[NSUserDefaults standardUserDefaults] valueForKey:@"GOOGLE_MAP_LANG_CODE"];
...
-(NSCalendarIdentifier)getCalendarIdentifire
{
    NSString* langCode = self.getGoogleMapLngCode;
    if([langCode isEqualToString:@"ar"]){
        
        return NSCalendarIdentifierIslamic;
        
    }else if([langCode isEqualToString:@"fa"]){
...
{   
    ...
    _gregorian = [[NSCalendar alloc] initWithCalendarIdentifier: [self getCalendarIdentifire]];
    _components = [[NSDateComponents alloc] init];
    _formatter = [[NSDateFormatter alloc] init];
    _formatter.dateFormat = @"yyyy-MM-dd";
//    _formatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"en"];
...
//    NSLog(@"Identifire:%@",_locale.localeIdentifier);
    
    
    NSNumberFormatter *formatter = [NSNumberFormatter new];
    formatter.locale = _locale;
    
    NSString *numStr = @([self.gregorian component:NSCalendarUnitDay fromDate:date]).stringValue;
    
    NSNumberFormatter *f = [[NSNumberFormatter alloc] init];
    f.numberStyle = NSNumberFormatterDecimalStyle;
    NSNumber *num = [f numberFromString:numStr];
    
    numStr = [numStr stringByReplacingOccurrencesOfString:num.stringValue withString:[formatter stringFromNumber:num]];
    
    cell.titleLabel.text = [self.dataSourceProxy calendar:self titleForDate:date] ?: numStr;
