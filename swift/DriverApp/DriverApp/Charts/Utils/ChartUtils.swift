...
open class ChartUtils
{
    ...
    open class func drawText(context: CGContext, text: String, point: CGPoint, align: NSTextAlignment, attributes: [String : AnyObject]?)
    {
        ...
        // For left and right edges values frame for long values
        if point.x < 0 {
            point.x = 2.5
        }else if (point.x + text.size(attributes: attributes).width ) > (Application.screenSize.width - 20){
            point.x = (Application.screenSize.width - 20) - ((text.size(attributes: attributes).width) + 6)
        }
        
        let rect: CGRect = CGRect(x:point.x - 2.5, y: point.y - 3, width: text.size(attributes: attributes).width + 5, height: text.size(attributes: attributes).height + 3)
        
        UIColor.UCAColor.AppThemeColor_Dark.set()
        UIRectFill(rect)
        
        (text as NSString).draw(in: UIEdgeInsetsInsetRect(rect, UIEdgeInsets(top: 1, left: 2, bottom: 1, right: 2
