...
open class BarChartDataEntry: ChartDataEntry
{
    ...
    fileprivate var _ranges: [CRange]?
    ...
    open func calcRanges()
    {
        ...
        for i in 0 ..< values!.count
        {
            ...
            if value < 0
            {
                _ranges?.append(CRange(from: negRemain, to: negRemain - value))
                negRemain -= value
            }
            else
            {
                _ranges?.append(CRange(from: posRemain, to: posRemain + value))
                posRemain += value
            }
        }
    }
