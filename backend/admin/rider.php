...
if($keyword != ''){
    if($option != '') {
        ...
        if($eStatus != ''){
            $ssql.= " AND ".stripslashes($option_new)." LIKE '%".$generalobjAdmin->clean($keyword)."%' AND eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        }else {
            $ssql.= " AND ".stripslashes($option_new)." LIKE '%".$generalobjAdmin->clean($keyword)."%'";
        }
    } else {
        if($eStatus != ''){
            $ssql.= " AND (concat(vName,' ',vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%' OR vEmail LIKE '%".$generalobjAdmin->clean($keyword)."%' OR vPhone LIKE '%".$generalobjAdmin->clean($keyword)."%') AND eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        } else {
            $ssql.= " AND (concat(vName,' ',vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%' OR vEmail LIKE '%".$generalobjAdmin->clean($keyword)."%' OR vPhone LIKE '%".$generalobjAdmin->clean($keyword)."%')";
        }
    }
...
$sql = "SELECT COUNT(iUserId) AS Total FROM register_user WHERE 1=1 $estatusquery $ssql $rdr_ssql";
...
if(!empty($eStatus)) { 
    ...
    $sql = "SELECT iUserId,CONCAT(vName,' ',vLastName) AS name, vEmail, vPhone AS mobile,tRegistrationDate,eStatus FROM register_user WHERE 1=1 $esql $ssql $rdr_ssql $ord LIMIT $start, $per_page";
                                                ...
                                                <tbody>
													<?php  if(!empty($data_drv)) { 
                                                        for($i=0;$i<count($data_drv);$i++) {
                                                            ?>
														<tr class="gradeA">
														<td align="center" style="text-align:center;"><input type="checkbox" id="checkbox" name="checkbox[]"  value="<?php  echo $data_drv[$i]['iUserId']; ?>" />&nbsp;</td>
															<td>
															...
															<td data-order="<?=$data_drv[$i]['iUserId']; ?>"><?php  echo $generalobjAdmin->DateTime($data_drv[$i]['tRegistrationDate']) ?></td>
