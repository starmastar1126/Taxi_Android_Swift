...
Class General {
    ...
    function fileupload($filepath, $vfile, $vfile_name, $prefix = '', $vaildExt = "mp3,wav") {
        ...
        if (!empty($vfile_name) && is_file($vfile)) {
            ...
            $file = implode("_", $tmp1);
            $ext = $tmp[count($tmp) - 1];
			//$value['base_url'] = $filepath;
			//$value['file'] = $file;
			//$value['ext'] = $ext;
			
			
            $vaildExt_arr = explode(",", strtoupper($vaildExt));
			//$value['vaildExt_arr'] = $vaildExt_arr;
            if (in_array(strtoupper($ext), $vaildExt_arr)) {

                $vfilefile = $file . "_" . date("YmdHis") . "." . $ext;
                $ftppath1 = $filepath . $vfilefile;
                //error_log("ftpPath : ".$ftppath1."\n", 3, "/var/www/html/my.log");
                
                // $value['ftppath1'] = $ftppath1;
                ...
                // try {
                    if (!copy($vfile, $ftppath1)) {
                        ...
                        $msg = $langage_lbl['LBL_FILE_UPLOADED_UNSUCCESS_MSG'];
                        $errorflag = "1";
                        $value['msg'] = "LBL_FILE_UPLOADED_UNSUCCESS_MSG";
                    } else {
                        $msg = $langage_lbl['LBL_FILE_UPLOADED_SUCCESS_MSG'];
                        $errorflag = "0";
                        $value['msg'] = "LBL_FILE_UPLOADED_SUCCESS_MSG";
                    }
                    
                // } catch (Exception $e) {
                //     error_log($e->getMessage(), 3, "/var/www/html/my.log");
                //     $value['error_log'] = $e->getMessage();
                // }
            } else {
                $vfilefile = '';
                $msg = $langage_lbl['LBL_FILE_EXT_VALID_ERROR_MSG'] . $vaildExt . "!!!";
