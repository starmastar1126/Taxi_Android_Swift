...
public class SimpleRatingBar extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (desiredStarSize != Integer.MAX_VALUE) {
                // rider specified a specific star size, so there is a desired width
                ...
                // rider specified a max star size, so there is a desired width
    ...
    private void drawStar(Canvas canvas, float x, float y, float filled, Gravity gravity) {
        ...
        // Note: below, currentStarSize*0.02f is a minor correction so the rider won't see a vertical black line in between the fill and empty color
        if (gravity == Gravity.Left) {
            // color star fill
    ...
    private void setNewRatingFromTouch(float x, float y) {
        ...
        // reduce the width to allow the rider reach the top and bottom values of rating (0 and numberOfStars)
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
     ...
     * If provided value is true, touch events will be deactivated, and thus rider interaction will be deactivated.
     *
     * @param indicator
     */
    ...
    public interface OnRatingBarChangeListener {
         ...
         * fromUser parameter to distinguish rider-initiated changes from those
         * that occurred programmatically. This will not be called continuously
         * while the rider is dragging, only when the rider finalizes a rating by
         * lifting the touch.
         *
         * @param simpleRatingBar The RatingBar whose rating has changed.
         * @param rating          The current rating. This will be in the range
         *                        0..numStars.
         * @param fromUser        True if the rating change was initiated by a rider's
